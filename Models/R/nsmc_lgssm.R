...
nsmc_lgssm <- function(xOld, obs, tau, lambda, sigmaY, M){
  ...
  for(i in 1:Nparticles){
    ...
    for (j in (d-1):1){
      lW <- -1.00*lambda*(x[i, j+1] - q[[i]]$x[, j])^2
      W <- exp(lW - max(lW))
      # resampling
...
nsmc_inner_lgssm <- function(xinnerOld, obs, tau, lambda, sigmaY, M){
  ...
  # j=1
  j <- 1
  xinner[, j] <- 1.00*xinnerOld[j] + rnorm(M)/sqrt(tau)
  # weights
  lW <- -1.00*(obs[j] - xinner[, j])^2/sigmaY - 1.00*log(2*pi*sigmaY)
  max.lW <- max(lW)
  W <- exp(lW - max(lW))
  ...
  for(j in 2:d){
    # propose
    xinner[, j] <- (1.00*tau*xinnerOld[j] + lambda*xinner[, j-1])/(tau+lambda) + rnorm(M)/sqrt(tau+lambda)
    # weights
    lW <- -1.00*(obs[j] - xinner[, j])^2/sigmaY - 1.00*log(2*pi*sigmaY)
    max.lW <- max(lW)
    W <- exp(lW - max(lW))
