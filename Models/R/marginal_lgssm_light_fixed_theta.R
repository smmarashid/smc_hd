...
marginal_lgssm_light <- function(i, u, nv, ci, W, Nparticles, theta, lambda, tau, x, history){
  ...
  for (n in 1:(theta*Nparticles)) {
    ...
    for (j in 2:(2*nv)) {
      ...
      integral_per_dimension[, j] <- -1.00*(tau+lambda)*(mx[dimension] - 1.00*tau*history[, dimension]/(tau+lambda))^2 -
        1.00*lambda*tau*(mx[dimension-1] * history[, dimension])/(tau+lambda)
    }
    if(ci[1] == 1){
      integral_per_dimension[, 1] <- -1.00*tau*(mx[1] - 1.00*history[, 1])^2
    } else {
      integral_per_dimension[, 1] <- -1.00*(tau+lambda)*(mx[ci[1]] - 1.00*tau*history[, ci[1]]/(tau+lambda))^2
    }
  ...
  lWmix <- - 1.00*lambda * (lambda *x[indices1, (ci[1]+nv-1)]^2/(tau+lambda) -
...
marginal_lgssm_light_vectorized <- function(i, u, nv, ci, W, Nparticles, theta, lambda, tau, x, history){
  ...
  if(u == 1){
    j <- 2*nv
    dimension <- (ci[1]:ci[2])[j]
    integral_per_dimension[, , j] <- -1.00*(tau+lambda)*outer(x[indices2, dimension], 1.00*tau*history[, dimension]/(tau+lambda), "-")^2 -
      1.00*lambda*tau*outer(x[indices1, dimension-1], history[, dimension], "*")/(tau+lambda)
  } else {
    for (j in 2:nv) {
      dimension <- (ci[1]:ci[2])[j]
      integral_per_dimension[, , j] <- -1.00*(tau+lambda)*outer(x[indices1, dimension], 1.00*tau*history[, dimension]/(tau+lambda), "-")^2 -
        1.00*lambda*tau*outer(x[indices1, dimension-1], history[, dimension], "*")/(tau+lambda)
    }
    for (j in (nv+2):(2*nv)) {
      dimension <- (ci[1]:ci[2])[j]
      integral_per_dimension[, , j] <- -1.00*(tau+lambda)*outer(x[indices2, dimension], 1.00*tau*history[, dimension]/(tau+lambda), "-")^2 -
        1.00*lambda*tau*outer(x[indices2, dimension-1], history[, dimension], "*")/(tau+lambda)
    }
    j <- nv+1
    dimension <- (ci[1]:ci[2])[j]
    integral_per_dimension[, , j] <- -1.00*(tau+lambda)*outer(x[indices2, dimension], 1.00*tau*history[, dimension]/(tau+lambda), "-")^2 -
  ...
  if(ci[1] == 1){
    integral_per_dimension[, , 1] <- -1.00*tau*outer(x[indices1, 1], 1.00*history[, 1], "-")^2
  } else {
    integral_per_dimension[, , 1] <- -1.00*(tau+lambda)*outer(x[indices1, ci[1]], 1.00*tau*history[, ci[1]]/(tau+lambda), "-")^2
  }
  ...
  integral_right <- ifelse(all(integral_right < .Machine$double.eps), rep(0, Nparticles), log(integral_right))
  integral_left <- ifelse(all(integral_left < .Machine$double.eps), rep(0, Nparticles), log(integral_left))
  lWmix <- -1.00*lambda * (lambda *x[indices1, (ci[1]+nv-1)]^2/(tau+lambda) - 2*x[indices1, (ci[1]+nv-1)] * x[indices2, (ci[1]+nv)]) +
