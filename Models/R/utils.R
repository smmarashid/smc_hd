...
w1_dist <- function(x, N){
  transport::wasserstein1d(x[1:N], x[N:length(x)])
  }

# KL divergence
kl_diver <- function(p, q){
  LaplacesDemon::KLD(p, q)$mean.sum.KLD
  }


## KL divergence between two mv-Gaussians
kl_div <- function(mu1, cov1, mu2, cov2) {
  d <- length(mu1) # Dimension of the distributions
  
  # calculate the KL divergence
  kl_divergence <- 0.5*(sum(diag(solve(cov2) %*% cov1)) +
                          t(mu2 - mu1) %*% solve(cov2) %*% (mu2 - mu1) -
